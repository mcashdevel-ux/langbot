...
| `housekeeping` | `enabled`, `scratch_max_age_days`, `scratch_max_total_mb`, `checkpoint_keep_threads`, `checkpoint_keep_per_thread`, `checkpoint_max_mb`, `checkpoint_vacuum_on_start` | start-up disk sweep (see below) |
...
Three things would otherwise grow forever: scratch entries are never truncated and never
deleted; every start (and every `/new`) mints a checkpoint thread nobody resumes; and the
checkpointer stores a **full snapshot of the whole message history on every super-step**,
so a single long session is O(N²) on its own. On a machine that runs the agent for weeks,
all three fill the disk silently. (Measured here: one 18-hour session accounted for 87% of
a 10 GB checkpoint store — see `docs/sitrep-2026-09-23-checkpoint-db-lock.md`.)
...
The active thread is never deleted, but its *oldest* checkpoints are: resuming replays
from the newest one, so trimming older snapshots costs no state the user can reach. Recent
...
whole rewrite). `checkpoint_vacuum_on_start` does it once at start-up, on the warmup
thread, before the checkpointer is busy — that is safe, and it is what keeps the file
from staying at its high-water mark. For the same reason, a *running* session's
overhead is not reclaimed at all: that is what `scripts/vacuum_checkpoints.py` is for
(`--thread <id>` also trims that thread's history first).
...
components/
  ...
  housekeeping.py       # start-up sweep: old scratch, abandoned threads, fat histories
